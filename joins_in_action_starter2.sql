...
SELECT * FROM movies LEFT JOIN users ON movies.user_id = favorite_movie_id;
...
SELECT * FROM movies RIGHT JOIN users ON users.id = favorite_movie_id;
...
SELECT * FROM movies INNER JOIN users ON movies.user_id = favorite_movie_id;


-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 4 >>>>>>>>>>>>>>>>>>>>>>>
-- Determine which lead studio's movies are favorited by users the most.
-- EXPECTED RESULT: Disney
SELECT lead_studio, COUNT(*) FROM movies Join users on movies.user_id = favorite_movie_id GROUP BY lead_studio ORDER BY COUNT(*) DESC LIMIT 1;
